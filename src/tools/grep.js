...
function grepSync(pattern, filePath) {
  ...
  try {
    // 执行 grep 命令并获取输出
    const grepcmd = `grep ${pattern} -rw ${filePath} --exclude-dir=node_modules --exclude-dir=vendor --exclude-dir=.git --exclude=sbom.json --exclude=syft --exclude=cve.json --exclude=package.json --exclude=go.mod --exclude=go.sum`
    logger.Info(grepcmd)
